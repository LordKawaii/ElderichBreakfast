...
public class PlayerController : MonoBehaviour
{
    ...
    void InputContoller()
    {
        if (gameCon.isOnePlayer)
        {
            if (isLeftGuy)
            {
                if ((Input.GetAxis("Horizontal") > 0 || (canUseController ? gameCon.controllerP1.LeftStickX.Value > 0 : false)) && transform.position.x < 10)
        ...
        /*else
        {
            ...
            /*if (!isLeftGuy && transform.position.x > -20)
            {
